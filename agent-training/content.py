...
add('开场与目标', '90 分钟：手把手搭建多 Agent 系统',
    split(table(['前半程', '用时'], [[name, duration_text(seconds)] for name, seconds in CHAPTERS[:4]]),
          table(['后半程', '用时'], [[name, duration_text(seconds)] for name, seconds in CHAPTERS[4:]])) +
    p('你将得到两项能力：<b>按复杂度选方案</b>；<b>设计最小协作闭环</b>', 'takeaway'),
    '八章总计 90 分钟，包含投票、演示、练习与讨论，不额外添加隐藏的实操时长。前半程先通过能力演进认识推理机制，再判断何时需要 Agent；后半程把机制映射到角色、框架和工程接口。'
