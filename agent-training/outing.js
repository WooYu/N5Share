...
function makeOutingTrace(stage, weather, budget) {
  ...
  if (stage === 1) {
    ...
    emit('决策 Reasoning', '先确定天气条件', '先查询天气，决定应查看室内还是室外候选。以下决策均由教学规则模拟。');
    ...
    for (const place of suitable) {
      emit('决策 Reasoning', `检查${place.name}的费用`, `候选符合天气要求；还需确认总费用是否不超过 ${budget} 元。`);
    ...
    emit('初稿 Draft', '故意放入一份有问题的建议', '“去湖畔公园，门票 60 + 交通 40 = 100 元。”初稿漏算餐费，也没有核对天气。');
    ...
    emit('修订 Revision', '重新计算并检查候选', suitable.map(outingPrice).join('\n'));
    finish();
  } else if (stage === 7) {
    emit('分工 Delegation', '为两种资料设置独立分析角色', '天气角色负责天气与室内外条件；费用角色负责三项费用；协调者合并结果。这里只演示分工，原任务用一个工作流也足够。');
